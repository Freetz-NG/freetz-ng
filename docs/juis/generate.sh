...
for x in $(seq $LAB_HW_RANGE_RUN $LAB_HW_RANGE_END); do  [ "$x" -lt 248 ] 2>/dev/null && m="$(( $x - 72 ))" || m=$x; m="$m.$a"
                             env - $TOOLS/juis_check        HW=$x         Buildtype=1001  Version=$m  -a; done | tee fos-lab; done
#inh
echo -e '\n### FOS-Inhaus #################################################'
for a in ${INH_FW_VERSION[@]}; do
for x in $(seq $INH_HW_RANGE_RUN $INH_HW_RANGE_END); do  [ "$x" -lt 248 ] 2>/dev/null && m="$(( $x - 72 ))" || m=$x; m="$m.$a"
                             env - $TOOLS/juis_check        HW=$x         Buildtype=1000  Version=$m  -a; done | tee fos-inh; done
#sub
...
for x in $seq; do [ ${#x} != 4 ] && x="0$x"; x="${x::-2}.${x:2}";                        m="226.$a"
                             env - $TOOLS/juis_check --dect HW=154 DHW=$x Buildtype=1000  Version=$m  -a; done | tee dect-lab; done
#dect-inh
echo -e '\n### Dect-Inhaus ################################################'
for a in ${DEC_FW_VERSION[@]}; do
for x in $seq; do [ ${#x} != 4 ] && x="0$x"; x="${x::-2}.${x:2}";                        m="226.$a"
                             env - $TOOLS/juis_check --dect HW=154 DHW=$x Buildtype=1001  Version=$m  -a; done | tee dect-inh; done
#dect-sub
