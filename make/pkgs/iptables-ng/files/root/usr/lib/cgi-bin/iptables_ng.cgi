...
if [ -r /mod/etc/conf/iptables_ng.cfg ]; then
	. /mod/etc/conf/iptables_ng.cfg
elif [ -r /mod/etc/default.iptables_ng/iptables_ng.cfg ]; then
	. /mod/etc/default.iptables_ng/iptables_ng.cfg
elif [ -r /etc/default.iptables_ng/iptables_ng.cfg ]; then
	. /etc/default.iptables_ng/iptables_ng.cfg
fi

: ${IPTABLES_NG_ENABLED:=no}
: ${IPTABLES_NG_AUTOSAVE_ON_STOP:=no}
: ${IPTABLES_NG_MODSAVE_ON_SAVE:=no}
...
ACTION_RC=0
SERVICE_RUNNING=0
SAVEFILE_V4=/var/tmp/flash/iptables_ng_rules.v4
SAVEFILE_V6=/var/tmp/flash/iptables_ng_rules.v6
FWD_PERSIST_FILE=/var/tmp/flash/iptables_ng_forwarding.conf
...
table_status() {
	...
	errfile="$(mktemp /tmp/iptables_ng.err.XXXXXX)"
	...
	case "$u" in
		*\?*)
			...
			for kv in $query; do
				...
				key="${kv%%=*}"
				case "$key" in
					action|family|view|ajax|fwd_family|fwd_if|fwd_val|fwd_persist|edit_family|edit_table|edit_chain|enabled|autosave_on_stop|modsave_on_save|chain_rules_*)
						continue
						;;
				esac
				[ -n "$keep" ] && keep="${keep}&"
...
run_modsave_flash() {
	if command -v modsave >/dev/null 2>&1; then
		modsave flash >/tmp/iptables_ng_modsave.log 2>&1
		return $?
	fi
	[ -x /usr/bin/modsave ] || return 1
	/usr/bin/modsave flash >/tmp/iptables_ng_modsave.log 2>&1
	return $?
}
...
fwd_persist_set() {
	...
	tmp="$(mktemp /tmp/iptables_ng_fwdpersist.XXXXXX)" || return 1
...
run_action() {
	...
	case "$ACTION" in
		save|restore)
			if /mod/etc/init.d/rc.iptables-ng "$ACTION" "${FAMILY:-all}" >/tmp/iptables_ng_action.log 2>&1; then
				ACTION_RC=0
				ACTION_MSG="$(lang de:"Aktion erfolgreich" en:"Action successful"): $(action_label "$ACTION") ($(family_label "${FAMILY:-all}"))"
				if [ "$ACTION" = save ] && [ "$IPTABLES_NG_MODSAVE_ON_SAVE" = yes ]; then
					if run_modsave_flash; then
						ACTION_MSG="$ACTION_MSG; $(lang de:"Flash gespeichert" en:"Flash saved")"
					else
						ACTION_RC=1
						ACTION_MSG="$ACTION_MSG; $(lang de:"modsave fehlgeschlagen" en:"modsave failed") - $(tr '\n' ' ' </tmp/iptables_ng_modsave.log)"
					fi
				fi
			else
				ACTION_RC=1
				ACTION_MSG="$(lang de:"Aktion fehlgeschlagen" en:"Action failed"): $(action_label "$ACTION") ($(family_label "${FAMILY:-all}")) - $(tr '\n' ' ' </tmp/iptables_ng_action.log)"
			fi
			;;
		set_fwd)
			...
			if set_iface_forwarding "$fwd_family" "$fwd_if" "$fwd_val" >/tmp/iptables_ng_action.log 2>&1; then
				# If this iface is marked persistent, update its stored state and commit it.
				if fwd_persist_update_if_enabled "$fwd_family" "$fwd_if" "$fwd_val" >/dev/null 2>&1; then
					run_modsave_flash >/dev/null 2>&1 || true
				fi
				ACTION_RC=0
		...
		set_fwd_persist)
			...
			if [ "$fwd_persist" = 1 ]; then
				...
				case "$fwd_state" in
					on) fwd_persist_set "$fwd_family" "$fwd_if" 1 >/tmp/iptables_ng_action.log 2>&1 ;;
					off) fwd_persist_set "$fwd_family" "$fwd_if" 0 >/tmp/iptables_ng_action.log 2>&1 ;;
					*)
						echo "invalid forwarding state: $fwd_state" >/tmp/iptables_ng_action.log
						false
						;;
				esac
				if [ "$?" -eq 0 ]; then
					if ! run_modsave_flash >/tmp/iptables_ng_action.log 2>&1; then
						ACTION_RC=1
						ACTION_MSG="$(lang de:"modsave fehlgeschlagen" en:"modsave failed") - $(tr '\n' ' ' </tmp/iptables_ng_action.log)"
					else
						ACTION_RC=0
						ACTION_MSG="$(lang de:"Forwarding persistiert" en:"Forwarding persisted"): $(family_label "$fwd_family") $fwd_if=$fwd_state"
					fi
				else
					ACTION_RC=1
				...
				if fwd_persist_unset "$fwd_family" "$fwd_if" >/tmp/iptables_ng_action.log 2>&1; then
					if ! run_modsave_flash >/tmp/iptables_ng_action.log 2>&1; then
						ACTION_RC=1
						ACTION_MSG="$(lang de:"modsave fehlgeschlagen" en:"modsave failed") - $(tr '\n' ' ' </tmp/iptables_ng_action.log)"
					else
		...
		save_chain)
			...
			elif apply_chain_rules "$edit_family" "$edit_table" "$edit_chain" "$chain_payload" >/tmp/iptables_ng_action.log 2>&1; then
				ACTION_RC=0
...
<br>
<input type='hidden' name='modsave_on_save' value='no'>
<input id='ia_ms' type='checkbox' name='modsave_on_save' value='yes'$( [ "$IPTABLES_NG_MODSAVE_ON_SAVE" = yes ] && echo " checked")><label for='ia_ms'> $(lang de:"Beim Speichern zusaetzlich modsave flash ausfuehren" en:"Run modsave flash when saving rules")</label>
</p>
